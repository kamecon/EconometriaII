...
rer2 <- get_bis(datasets$url[datasets$name == "Effective exchange rate indices (daily, horizontal time axis)"], quiet = TRUE)
...
#Gráfico con mayor ventana temporal (1964-2020)
rer %>%
  dplyr::filter(type=="Real" & reference_area=="Spain" & eer_basket=="N") %>%
  mutate(date = as.Date(as.yearmon(date))) %>%
  select(date, obs_value) %>% 
